...
import { SampleEssayFormModal } from "./SampleEssayFormModal";
...
export function SampleEssaysManager() {
  ...
  const handleEdit = (essay: SampleEssay) => { setEditingEssay(essay); setModalOpen(true); };
  const handleCreate = () => { setEditingEssay(null); setModalOpen(true); };
  ...
  return (
    <div>
            ...
            <tbody className="divide-y divide-slate-100">
              {essays.map((essay) => (
                <tr key={essay._id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-center text-slate-500">{essay.viewCount}</td>
                  <td className="px-4 py-3 text-center">
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex justify-end gap-2">
                      <button onClick={() => handleEdit(essay)}
      ...
      <SampleEssayFormModal isOpen={modalOpen} onClose={() => setModalOpen(false)} essay={editingEssay} />
      <ConfirmDialog
        isOpen={!!deleteId}
        title="Xóa bài mẫu"
